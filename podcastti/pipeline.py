...
from podcastti.rss_generator import add_new_episode, load_episodes, generate_rss_xml, save_episodes

async def run_pipeline():
    print("=" * 60)
    print("🎙️ INICIANDO PIPELINE AUTOMÁTICO DO PODCAST TI")
    print("=" * 60)
    
    # Coleta links de episódios existentes para deduplicação
    ...
    if not news:
        ...
        return
    print(f"[OK] {len(news)} matérias coletadas e filtradas com sucesso.")
    
    print("\n[Passo 2/4] Criando roteiro didático para Léo & Sara...")
    script_data = generate_script_with_ai(news)
    print(f"[OK] Roteiro criado: '{script_data['title']}'")
    
    print("\n[Passo 3/4] Registrando episódio no histórico...")
    ...
    print("\n[Passo 4/4] Sintetizando vozes neurais e áudio MP3...")
    ...
    print(f"📌 Título do Episódio: {new_ep['title']}")
    print(f"🎵 Arquivo de Áudio MP3: {filepath} ({file_size} bytes)")
    print(f"📡 Feed RSS Atualizado: rss.xml")
    print("=" * 60)
